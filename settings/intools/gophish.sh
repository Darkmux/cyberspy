...
function installing() {
	...
	echo -e ${red}"
[${green}*${red}] ${green}Installing gophish..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install golang
        git clone https://github.com/gophish/gophish ${opt}/gophish
        cd ${opt}/gophish
        rm config.json
        cp ${settings}/config/config.json ${opt}/gophish
        go build
        cp ${execute}/gophish ${bin}
        chmod 777 ${bin}/gophish
        echo -e ${red}"

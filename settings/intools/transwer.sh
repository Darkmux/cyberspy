...
# ==============================================
function installing() {
    if [ ! -x ${bin}/transwer ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing transwer..."${white}
        yes|pkg update && yes|pkg upgrade
        yes|pkg install translate-shell bat
        git clone https://github.com/Darkmux/transwer ${opt}/transwer
        cd ${opt}/transwer
	chmod 777 *.sh
        cp ${execute}/transwer ${bin}
        chmod 777 ${bin}/transwer
        echo -e ${red}"

...
# ==============================================
function installing() {
    if [ ! -x ${bin}/slowloris ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing slowloris..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        pip install --upgrade pip
        git clone https://github.com/gkbrk/slowloris ${opt}/slowloris
        cd ${opt}/slowloris
	python3 setup.py install
        cp ${execute}/slowloris ${bin}
        chmod 777 ${bin}/slowloris
        echo -e ${red}"

...
# ==============================================
function installing() {
    if [ ! -x ${bin}/hash-id ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing hash-id..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/blackploit/hash-identifier ${opt}/hash-identifier
        cp ${execute}/hash-id ${bin}
        chmod 777 ${bin}/hash-id
        echo -e ${red}"

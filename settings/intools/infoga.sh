...
# ==============================================
function installing() {
    if [ ! -x ${bin}/infoga ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing infoga..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/m4ll0k/Infoga ${opt}/Infoga
        cd ${opt}/Infoga
        python3 -m pip install -r requirements.txt
	python3 setup.py install
        cp ${execute}/infoga ${bin}
        chmod 777 ${bin}/infoga
        echo -e ${red}"

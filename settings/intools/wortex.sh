...
# ==============================================
function installing() {
    if [ ! -x ${bin}/wortex ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing wortex..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        yes|pkg install wget
        python3 -m pip install --upgrade pip
        wget https://raw.githubusercontent.com/Darkmux/phoenix-packages/main/tools/wortex.zip -O ${settings}/lctools/wortex.zip
	cd ${settings}/lctools
	unzip wortex.zip
	mv wortex ${opt}
        rm -rf wortex.zip
        cd ${opt}/wortex
        cp ${execute}/wortex ${bin}
        chmod 777 ${bin}/wortex
        echo -e ${red}"

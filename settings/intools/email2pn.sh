...
# ==============================================
function installing() {
    if [ ! -x ${bin}/email2pn ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing email2pn..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/martinvigo/email2phonenumber ${opt}/email2phonenumber
        cd ${opt}/email2phonenumber
        python3 -m pip install -r requirements.txt
        cp ${execute}/email2pn ${bin}
        chmod 777 ${bin}/email2pn
        echo -e ${red}"

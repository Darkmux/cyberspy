...
function installing() {
	...
	echo -e ${red}"
[${green}*${red}] ${green}Installing pwnedornot..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        python3 -m pip install html2text
        git clone https://github.com/thewhiteh4t/pwnedOrNot ${opt}/pwnedOrNot
        cd ${opt}/pwnedOrNot
        chmod 777 *.sh
        bash install.sh
        cp ${execute}/pwnedornot ${bin}
        chmod 777 ${bin}/pwnedornot
        echo -e ${red}"
[${green}√${red}] ${green}Installation Finished, Please Execute:${white}

pwnedornot <ARGUMENTS>

...
# ==============================================
function installing() {
    if [ ! -x ${bin}/qrlink ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing qrlink..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        pip install --upgrade pip
        python3 -m pip install Pillow
        git clone https://github.com/Darkmux/qrlink ${opt}/qrlink
        cd ${opt}/qrlink
	chmod 777 *.sh
        python3 -m pip install -r requirements.txt
        cp ${execute}/qrlink ${bin}
        chmod 777 ${bin}/qrlink
        echo -e ${red}"

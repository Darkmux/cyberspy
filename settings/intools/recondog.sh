...
# ==============================================
function installing() {
    if [ ! -x ${bin}/recondog ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing recondog..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/s0md3v/ReconDog ${opt}/ReconDog
        cd ${opt}/ReconDog
        python3 -m pip install -r requirements.txt
        cp ${execute}/recondog ${bin}
        chmod 777 ${bin}/recondog
        echo -e ${red}"

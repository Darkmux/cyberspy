...
# ==============================================
function installing() {
    if [ ! -x ${bin}/saycheese ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing saycheese..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install wget php httrack openssh curl 
        wget https://raw.githubusercontent.com/Darkmux/phoenix-packages/main/tools/saycheese.zip -O ${settings}/lctools/saycheese.zip
	cd ${settings}/lctools
	unzip saycheese.zip
	mv saycheese ${opt}
        rm -rf saycheese.zip

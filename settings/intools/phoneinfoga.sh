...
function installing() {
	...
	echo -e ${red}"
[${green}*${red}] ${green}Installing phoneinfoga..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        pip install --upgrade pip
        git clone https://github.com/Darkmux/PhoneInfoga ${opt}/PhoneInfoga
        cd ${opt}/PhoneInfoga
	python3 -m pip install -r requirements.txt
	python3 -m pip install config
        cp ${execute}/phoneinfoga ${bin}
        chmod 777 ${bin}/phoneinfoga
        echo -e ${red}"
[${green}√${red}] ${green}Installation Finished, Please Execute:${white}

phoneinfoga <ARGUMENTS>

...
# ==============================================
function installing() {
    if [ ! -x ${bin}/fbuserid ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing fbuserid..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install curl
        git clone https://github.com/Darkmux/fbuserid ${opt}/fbuserid
        cd ${opt}/fbuserid
	chmod 777 *.sh
        cp ${execute}/fbuserid ${bin}
        chmod 777 ${bin}/fbuserid
        echo -e ${red}"

...
# ==============================================
function installing() {
    if [ ! -x ${bin}/redhawk ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing redhawk..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install php
        git clone https://github.com/Tuhinshubhra/RED_HAWK ${opt}/RED_HAWK
        cp ${execute}/redhawk ${bin}
        chmod 777 ${bin}/redhawk
        echo -e ${red}"

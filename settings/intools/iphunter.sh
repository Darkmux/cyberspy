...
# ==============================================
function installing() {
    if [ ! -x ${bin}/iphunter ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing iphunter..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        yes|pkg install wget
        python3 -m pip install --upgrade pip
        wget https://raw.githubusercontent.com/Darkmux/phoenix-packages/main/tools/iphunter.zip -O ${settings}/lctools/iphunter.zip
	cd ${settings}/lctools
	unzip iphunter.zip
	mv iphunter ${opt}
        rm -rf iphunter.zip
	...
	python3 -m pip install -r requirements.txt
        cp ${execute}/iphunter ${bin}
        chmod 777 ${bin}/iphunter
        echo -e ${red}"

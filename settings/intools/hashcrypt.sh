...
# ==============================================
function installing() {
    if [ ! -x ${bin}/hashcrypt ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing hashcrypt..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/Darkmux/hashcrypt ${opt}/hashcrypt
        cd ${opt}/hashcrypt
        python3 -m pip install -r requirements.txt
        cp ${execute}/hashcrypt ${bin}
        chmod 777 ${bin}/hashcrypt
        echo -e ${red}"

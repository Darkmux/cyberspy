...
# ==============================================
function installing() {
    if [ ! -x ${bin}/protosint ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing protosint..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/pixelbubble/ProtOSINT ${opt}/ProtOSINT
        cp ${execute}/protosint ${bin}
        chmod 777 ${bin}/protosint
        echo -e ${red}"

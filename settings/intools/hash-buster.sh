...
# ==============================================
function installing() {
    if [ ! -x ${bin}/buster ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing hash-buster..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python
        python3 -m pip install --upgrade pip
        git clone https://github.com/s0md3v/Hash-Buster ${opt}/Hash-Buster
        ln -s ${opt}/Hash-Buster/hash.py ${bin}/buster
        chmod 777 ${bin}/buster
        echo -e ${red}"

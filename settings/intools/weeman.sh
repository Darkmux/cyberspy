...
# ==============================================
function installing() {
    if [ ! -x ${bin}/weeman ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing weeman..."${white}
        yes|pkg update && pkg upgrade
        yes|pkg install python2
        python2 -m pip install --upgrade pip
        git clone https://github.com/evait-security/weeman ${opt}/weeman
        cd ${opt}/weeman
	chmod 777 weeman.py
        cp ${execute}/weeman ${bin}
        chmod 777 ${bin}/weeman
        echo -e ${red}"
